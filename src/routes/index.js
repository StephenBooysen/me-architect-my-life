/**
 * @fileoverview Application routes for Express.js application.
 * 
...
/**
 * Configures and registers application routes with the Express application.
 * 
...
module.exports = (app, database) => {
    ...
    app.get("/api/focus-areas", async (req, res) => {
        try {
            // Get unique focus areas by name (to handle duplicates)
            const focusAreas = await database.all(
                `SELECT MIN(id) as id, name, category, month, theme, is_active, MIN(created_at) as created_at
       FROM focus_areas 
       WHERE is_active = 1 
