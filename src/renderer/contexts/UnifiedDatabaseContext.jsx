...
const apiCall = async (endpoint, method = 'GET', data = null) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(`API call failed: ${response.statusText}. Response: ${text}`);
  }
...
export function DatabaseProvider({ children }) {
  ...
  const getGoals = useCallback(async (type = null, parentId = null) => {
    if (isElectron) {
  ...
  const getGoalWithChildren = useCallback(async (id) => {
    if (isElectron) {
      const goal = await getRecord('SELECT * FROM goals WHERE id = ?', [id]);
      if (!goal) return null;
      
      const children = await getAllRecords('SELECT * FROM goals WHERE parent_id = ? ORDER BY created_at DESC', [id]);
  ...
  const getGoalHierarchy = useCallback(async (id) => {
    if (isElectron) {
      const hierarchy = [];
      let currentId = id;
      
      while (currentId) {
        const goal = await getRecord('SELECT * FROM goals WHERE id = ?', [currentId]);
        if (!goal) break;
  ...
  const updateGoalProgress = useCallback(async (id, progress) => {
    if (isElectron) {
      ...
      await runQuery('UPDATE goals SET progress = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [progress, id]);
      ...
      if (goal && goal.parent_id) {
        const siblings = await getAllRecords('SELECT progress FROM goals WHERE parent_id = ?', [goal.parent_id]);
        const avgProgress = siblings.reduce((sum, sibling) => sum + (sibling.progress || 0), 0) / siblings.length;
        
        await runQuery('UPDATE goals SET progress = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [Math.round(avgProgress), goal.parent_id]);
        
        // Continue up the chain
        const parent = await getRecord('SELECT * FROM goals WHERE id = ?', [goal.parent_id]);
        if (parent && parent.parent_id) {
          const parentSiblings = await getAllRecords('SELECT progress FROM goals WHERE parent_id = ?', [parent.parent_id]);
          const parentAvgProgress = parentSiblings.reduce((sum, sibling) => sum + (sibling.progress || 0), 0) / parentSiblings.length;
          
          await runQuery('UPDATE goals SET progress = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [Math.round(parentAvgProgress), parent.parent_id]);
        }
      }
